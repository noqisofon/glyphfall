...
- **レトロ2Dドット絵／スプライト・ハイブリッド（ADR-0009）**
  - 中央ウィンドウに 16×16 ピクセルのレトロドット絵（736×144pxテクスチャ）で街並みを鮮やかに描画
...
- **視界制限付き見下ろし街移動＆DQ式仲間追従（ADR-0008）**
  - 王都アルカンの街マップ（宿屋、酒場、商店、街路樹、噴水、裏路地、地下階段）を移動
  - `[WASD]` / 矢印キーで主人公（`@`）移動、直前の足跡を仲間たちがDQのように1列で追従
  - 扉の開閉、看板の閲覧、町人・衛兵・怪しい男との会話、地下階段や `[B]` キーによる戦闘ビューへのシームレスな移行
